...
volatile float pKp = 15.0;      //position mode PID values.
...
// *** STM32F401 PORT ***
float __attribute__((__aligned__(256))) lookup[LOOKUP_SIZE] = {
//Put lookup table here!
};

//retirer le const également dans le fichier .h (ou le remttre quand on passera en FLASH ;-)
...
// *** STM32F401 PORT ***
int cpr = LOOKUP_SIZE;                    // counts per rev
const float stepangle = aps/32.0;   // for step/dir interrupt: aps/32 is the equivalent of 1/32 microsteps

volatile float PA = aps;            // Phase advance...

const float iMAX = 1.0;
const float rSense = 0.150;

// *** STM32F401 PORT ***
// Sur STM32F4 avec la core Arduino, on utilise un PWM 10 bits via analogWriteResolution(10).
// On garde donc le calcul de uMAX en cohérence avec une pleine échelle 1023.
volatile int uMAX = (1023/3.3)*(iMAX*10*rSense);   // *** MODIF STM32F4 ***
// NOTE: l’original utilisait 255 pour 8 bits ou 1023 pour 10 bits.
// Vérifie que analogFastWrite() utilise bien une résolution 10 bits sur STM32.
