...
// Si tu veux du vrai stockage flash, utilise EEPROM_emul STM32 ou une lib dédiée.
#include "FlashStorage_stub.h"  // *** MODIF STM32F4 ***
...
void configureStepDir() {
  ...
  attachInterrupt(digitalPinToInterrupt(step_pin), stepInterrupt, RISING);  // *** STM32F401 PORT ***
  attachInterrupt(digitalPinToInterrupt(dir_pin), dirInterrupt, CHANGE);    // *** STM32F401 PORT ***
}

void configureEnablePin() {
  pinMode(enable_pin, INPUT);
  attachInterrupt(digitalPinToInterrupt(enable_pin), enableInterrupt, CHANGE);  // *** STM32F401 PORT ***
}
...
void dirInterrupt() {
  // *** STM32F401 PORT ***
  // Remplacement REG_PORT_IN0 & PORT_PA11 par digitalRead(dir_pin)
  if (digitalRead(dir_pin)) dir = false;
  else dir = true;
}

// *** STM32F401 PORT ***
// HardwareTimer pour remplacer TC5 SAMD21 (Fs = 6500 Hz)
extern HardwareTimer *controlTimer;

// *** STM32F401 PORT ***
// Implémentations des fonctions TC SAMD21 remplacées par HardwareTimer
void setupTCInterrupts() {
  // *** OBSOLETE sur STM32F401 ***
  // Le timer est déjà configuré dans setup() avec HardwareTimer
  SerialUSB.println("setupTCInterrupts(): Timer already configured in setup()");
}

void enableTCInterrupts() {
  if (controlTimer) {
    controlTimer->resume();  // *** MODIF STM32F4 *** Démarre la boucle de contrôle
    SerialUSB.println("Closed loop ENABLED (6500 Hz)");
  }
}

void disableTCInterrupts() {
  if (controlTimer) {
    controlTimer->pause();   // *** MODIF STM32F4 *** Arrête la boucle de contrôle
...
void enableInterrupt() {
  // *** STM32F401 PORT ***
  // Remplacement REG_PORT_IN0 & PORT_PA14 par digitalRead(enable_pin)
...
void output(float theta, int effort) {
  // [code inchangé - utilise les macros IN_x_HIGH/LOW et analogFastWrite]
...
// *** STM32F401 PORT ***
// Remplacement FlashStorage SAMD21 par accès direct flash STM32.
// On réserve 64KB à la fin de la flash (secteur 7: 0x080E0000-0x080FFFFF sur F401).

/*

#include <libmaple_flash.h>  // *** STM32F401 PORT *** (Arduino STM32 core)
#include <string.h>

static FlashClass flash;  // *** MODIF *** FlashClass est maintenant STM32 FlashClass
...
static const void * page_ptr = (const void*)0x080E0000;  // *** STM32F401 PORT *** Secteur 7 flash F401

static void write_page()
{
  // Debug print conservé
  ...
  // *** STM32F401 PORT ***
  // Écriture flash STM32 : unlock + erase + program + lock
  flash_unlock();  // Déverrouille flash
  
  // Erase secteur 7 (64KB) si nécessaire
  FLASH_EraseSector(7, FLASH_VOLTAGE_RANGE_3);  // Vcc=3.3V
  
  // Écriture page par page (32-bit words)
  uint32_t *src = (uint32_t*)page;
  uint32_t *dst = (uint32_t*)page_ptr;
  for(int i = 0; i < (sizeof(page)/4); i++) {
    flash_program_word((uint32_t)&dst[i], src[i]);
  }
  
  flash_lock();  // Verrouille flash
}
...
static void store_lookup(float lookupAngle)
{
  ...
  write_page();
  page_ptr = (uint8_t*)page_ptr + sizeof(page);  // Incrémente pointeur
  page_count = 0;
...
/* Version RAM simplifiée de calibrate*/
void calibrate() {   // Calibration routine - *** STM32F401 PORT : RAM only ***
  ...
  int fullStepReadings[spr];  // 200 éléments pour 200 steps/rev
  ...
  // Step through all full step positions, recording encoder readings
  for (int x = 0; x < spr; x++) {
    encoderReading = 0;
    delay(20);
    lastencoderReading = readEncoder();
        
    // Average multiple readings at each step
    ...
    encoderReading = encoderReading / avg;
    if (encoderReading > cpr) encoderReading -= cpr;
    else if (encoderReading < 0) encoderReading += cpr;
  ...
  // Find encoder zero position (iStart, jStart)
  for (int i = 0; i < spr; i++) {
    ticks = fullStepReadings[mod((i + 1), spr)] - fullStepReadings[mod((i), spr)];
    if (ticks < -15000) ticks += cpr;
    else if (ticks > 15000) ticks -= cpr;
  ...
  // *** STM32F401 PORT : REMPLACEMENT FLASH PAR RAM DIRECT ***
  // Génération directe de la table lookup[] en RAM (pas de page/flash)
  SerialUSB.println("Generating lookup table in RAM...");
  
  int lookupIndex = 0;
  for (int i = iStart; i < (iStart + spr + 1) && lookupIndex < LOOKUP_SIZE; i++) {
    ticks = fullStepReadings[mod((i + 1), spr)] - fullStepReadings[mod((i), spr)];
    ...
    if (ticks > 1) {
      if (i == iStart) {
        for (int j = jStart; j < ticks; j++) {
          lookup[lookupIndex++] = 0.001 * mod(1000 * ((aps * i) + ((aps * j) / float(ticks))), 360000.0);  
        }
      }
      else if (i == (iStart + spr)) {
        for (int j = 0; j < jStart; j++) {
      ...
      if (i == iStart) {
        for (int j = -ticks; j > jStart; j--) {
          lookup[lookupIndex++] = 0.001 * mod(1000 * (aps * i + (aps * ((ticks + j)) / float(ticks))), 360000.0);
        }
      }
      else if (i == iStart + spr) {
        for (int j = jStart; j > 0; j--) {
  ...
  // *** STM32F401 PORT : FIN - table lookup[] remplie directement en RAM ***
  SerialUSB.println("Calibration complete!");
  SerialUSB.println("*** STM32F401 PORT: lookup table stored in RAM (lost on reboot) ***");
  SerialUSB.println("Reboot to reload original lookup table or re-run 'c' command.");
}
...
// void setupTCInterrupts() {  
//   // *** STM32F401 PORT ***
//   // Remplacement COMPLET de la config TC5 SAMD21 par HardwareTimer STM32
//   // Cette fonction est maintenant un stub - la vraie config se fait dans setup() ou Timer_stm32.cpp
//   SerialUSB.println("*** STM32F401 PORT: setupTCInterrupts() est un stub ***");
//   SerialUSB.println("Utilise HardwareTimer dans setup() pour Fs=6500Hz");
// }

// void enableTCInterrupts() {   
//   // *** STM32F401 PORT ***
//   // Stub - la vraie implémentation dans Timer_stm32.cpp ou setup()
//   // Garde l'interface pour compatibilité
// }
